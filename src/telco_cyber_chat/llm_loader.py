# lg_app/src/app/llm_loader.py
import os, re, torch
from functools import lru_cache
from transformers import AutoTokenizer, AutoModelForCausalLM, BitsAndBytesConfig, pipeline

MODEL_ID = os.getenv("MODEL_ID", "fdtn-ai/Foundation-Sec-8B-Instruct")
GUARD_ID = os.getenv("GUARD_ID", "meta-llama/Llama-Guard-3-8B")
USE_GUARD = os.getenv("USE_GUARD", "true").lower() == "true"

HF_TOKEN = os.getenv("HUGGINGFACE_HUB_TOKEN") or os.getenv("HF_TOKEN") or os.getenv("LLM_TOKEN")
...
def _get_guard():
    ...
    return pipeline(
        "text-generation", model=mdl, tokenizer=tok,
        do_sample=False, num_beams=1, top_p=1.0,
        max_new_tokens=128, return_full_text=True,
        pad_token_id=tok.pad_token_id,
    )
...
def ask_secure(question: str, *, context: str = "", role: str = "end_user",
    ...
    decoding["return_full_text"] = False
    if seed is not None:
        try:
            device = "cuda" if torch.cuda.is_available() else "cpu"
            decoding["generator"] = torch.Generator(device=device).manual_seed(int(seed))
        except Exception:
            decoding["generator"] = torch.Generator().manual_seed(int(seed))
