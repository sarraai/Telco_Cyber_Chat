...
def parse_cvrf(xml_text: str) -> Dict:
    ...
    wa = [v for k,v in notes_by_title.items()
    ...
    for rem in _iter_by_name(root, "Remediation"):
        ...
        if (
            ...
            if not prod_ids:
                ...
                for pid in prod_ids:
                    fixed_rows.append({"product": prod_map.get(pid, pid),
                                       "fix": desc})
    # Dedup
    seen, dedup = set(), []
    for r in fixed_rows:
...
def _parse_html_table(table: Tag) -> Optional[Dict]:
    try:
        ...
        if headers:
            for r in rows:
                records.append({
                    headers[i]: (r[i] if i < len(headers) else "")
        ...
        try:
            ...
            if not result["bug_ids"]:
                ...
                result["bug_ids"] = sorted(
                    set([m.group(0).upper() for m in bug_re.finditer(all_text)])
                )
                    ...
                    if parsed and (parsed["columns"] or parsed["rows"]):
                        ...
                        for row in parsed.get("rows",[]):
                            if len(row)>=2:
                                ...
                                text_rows.append({
                                    "product":current_product or "Fixed Software Table",
                                    "fix":row[0],
                                })
...
def transform_advisory(adv: Dict, token: str) -> Dict:
    ...
    fixed_sw_text = [
        row["fix"] for row in (fs.get("text_rows") or [])
        if row.get("fix")
    ]
...
def fetch_all_advisories(
    ...
    for y in range(end_year, start_year - 1, -1):
        try:
            ...
            print(f"[WARN] Fetch year {y} failed: {e}")
            continue
        ...
        for adv in year_list:
            ...
            if check_qdrant and url and url_already_ingested(url):
                # Already indexed → skip heavy work
                continue
            ...
            try:
                ...
                print(f"[WARN] Transform failed for {aid}: {e}")
            time.sleep(PAUSE_SEC)
...
def scrape_cisco(
    ...
    print(f"[CISCO] Scraped {len(docs)} advisories (documents).")
    return docs
